...
import Yandex from './youtube/yandex';
...
export class ScrapperService {
    ...
    async search(name: string):Promise<Track | Error>{
        ...
        if(provider == 'Yandex') return await this.yandex.searchURL(name)
        else return await this.youtube.search(name)
    }
    
    private getProvider(url:string): 'YouTube' | 'SoundCloud' | 'Yandex' |null{
        if(this.youtube.checkProvider(url)) return 'YouTube'
        if(this.soundcloud.checkProvider(url)) return 'SoundCloud'
        if(this.yandex.checkProvider(url)) return 'Yandex'
        return null
    }
}
